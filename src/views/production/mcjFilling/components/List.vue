    ...
    methods: {
      ExportData() {
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel([tHeader], data, '美瓷胶灌装线色系统计表', multiHeader, merges)
        })
      },
        ...
        getReportRulesList({
          ...
          getReportRulesList({
            ...
            if(interval > 3){
              ...
              this.columns = [
                {text: '时间段', width: '150px', colspan: true,fixed: true, data: [{width: '250px', text: '色系/灌装分类', name: 'project'}] }
              ]
              const columns = this.columns
              ...
              array1.forEach((a, b) => {
               ...
               colorStoreParticleSize(val, '美瓷胶灌装').then(res => {
                 if (res.flag) {
                   const data = res.data
